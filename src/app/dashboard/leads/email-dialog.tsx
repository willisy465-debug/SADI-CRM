...
import { generateLeadEmail } from "./actions"
...
export function EmailDialog({ leadId, leadName }: { leadId: string, leadName: string }) {
  ...
  async function handleGenerate(e: React.FormEvent) {
    ...
    setLoading(true)
    try {
      const content = await generateLeadEmail(leadId, objective)
          ...
          {!emailContent ? (
            <form onSubmit={handleGenerate} className="space-y-4 mt-4">
              <div className="space-y-2">
                <label className="text-sm text-slate-700">What is the objective of your email to {leadName}?</label>
                <Input 
                  value={objective}
                  onChange={(e) => setObjective(e.target.value)}
                  placeholder="e.g. Follow up on our meeting yesterday and propose a leadership course."
